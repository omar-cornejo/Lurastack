...
export default function LanguageSwitcher({ variant = "dark" }: LanguageSwitcherProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Icon icon="mdi:translate" className={isDark ? "text-[13px]" : "text-base"} />
        <span className={isDark ? "text-[11px]" : ""}>{activeLabel}</span>
        <Icon icon="mdi:chevron-down" className={isDark ? "text-[12px] opacity-70 ml-auto" : "text-sm opacity-70 ml-auto"} />
      </button>
      ...
      {open && (
        <div
          className={
            isDark
              ? "absolute right-0 top-full z-50 mt-0 min-w-[160px] border border-gray-700 bg-gray-900 py-1 text-xs text-gray-300 shadow-xl"
